document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkNotifications() {
        ...
        if (totalBalance < 0) {
            showToast("⚠️ Warning: Your balance is negative!", 'warning');
        }

        // 2. Budget Limit Warning (>=90% of income spent)
        if (totalIncome > 0) {
            const expenseRatio = totalExpenses / totalIncome;
            if (expenseRatio >= 0.9) {
                const budgetMessages = [
                    "⚠️ Heads up! You’re close to your budget limit!",
                    "🚨 Careful! Spending is about to exceed income!",
                    "💸 Alert! You're almost at 100% of your budget."
                ];
                const randomBudgetMessage = budgetMessages[Math.floor(Math.random() * budgetMessages.length)];
                showToast(randomBudgetMessage, 'warning');
            }
        }

        // 3. Saving Goal Progress Warning (>=80% of target saved)
        savingGoals.forEach(goal => {
            const progress = (goal.saved / goal.target) * 100;
            if (progress >= 80) {
                const savingMessages = [
                    `🎯 Almost there! ${progress.toFixed(0)}% saved for "${goal.name}"!`,
                    `🌟 Great job! ${progress.toFixed(0)}% of "${goal.name}" achieved!`,
                    `🔥 You're killing it! Only a bit more to reach "${goal.name}"!`
                ];
                const randomSavingMessage = savingMessages[Math.floor(Math.random() * savingMessages.length)];
                showToast(randomSavingMessage, 'success');
            }
        });
    }
    ...
    function showToast(message, type = 'info') {
        ...
        toast.style.backgroundColor = type === 'warning' ? '#e67e22' : '#2ecc71';
    ...
    function init() {
        ...
        checkNotifications(); // Call notifications check during initialization
    }
